...
module Cadenya
  module Resources
    class AiProviderKeys
      ...
      def list(workspace_id: nil, limit: nil, cursor: nil, prefix: nil, query: nil, promotional: nil, labels: nil, sort_order: nil, include_info: nil, request_options: nil)
        workspace_id = @core.resolve_default("workspaceId", "CADENYA_WORKSPACE_ID", workspace_id)
        _path = "/v1/workspaces/#{Util.path_param('workspaceId', workspace_id)}/ai_provider_keys"
        _query = {
          "limit" => limit,
          "cursor" => cursor,
          "prefix" => prefix,
        ...
        _items = (_data["items"] || []).map { |item| Types::AIProviderKey.from_json(item) }
        _next_cursor = (((_data)["pagination"] || {}))["nextCursor"].to_s
        Page.new(_items, _next_cursor) do |_c|
          list(workspace_id: workspace_id, limit: limit, cursor: _c, prefix: prefix, query: query, promotional: promotional, labels: labels, sort_order: sort_order, include_info: include_info, request_options: request_options)
        end
      end
